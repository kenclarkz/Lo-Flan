...
export const business = {
  ...
  // Stage 2: add `menu` (flavors, sizes, prices) and `ordering` here.
  order: {
    current: "Place an order through the Facebook Messenger button on the website (m.me/losflan).",
    byPhone: false,
  },
}
...
export function buildSystemInstruction(overrides = {}) {
  ...
  const facts = [
    ...
    `Email: ${biz.email}`,
    `Website: ${biz.website}`,
    `How customers currently order: ${biz.order.current}`,
  ]
  ...
  return [
    ...
    '- NEVER invent or guess prices, flavors, menu items, availability, lead times, or any business policy. These are not implemented yet.',
    '- If the caller asks about anything you do not know (including prices, menu, or availability), say you do not have that information yet and that the owner will follow up with them.',
    ...
    '- Do not make promises, guarantees, or commitments about orders or timing.',
    '',
